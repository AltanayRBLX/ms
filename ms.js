...
bot.on("guildDelete", function(guild) {
    ...
    //    });
    //});
    bot.user.setGame("MS | discord.gg/E5YRfu");
});
...
bot.on("message", function(message) {
    ...
    if (message.content.toLowerCase().indexOf("https") > -1 || message.content.toLowerCase().indexOf("http") > -1) {
        if (message.content.toLowerCase().indexOf("discord.gg/") > -1) {
            if (message.content.toLowerCase().indexOf("www") > -1) {
             if (message.author.bot == false) {
                message.react("😡")
                message.delete()
                message.channel.send("<@" + message.author.id + ">, **Reklam yapmamalısın!**");
              }
            }
        }
        else {
...
bot.login("NDEzMjk1MjA2Nzg0NTY1MjQ5.DWWutA.Ultq154glIL959k9tLulFN6yChA");
